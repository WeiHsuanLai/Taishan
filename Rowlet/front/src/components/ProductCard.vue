...
import { ref } from 'vue' // 引入 Vue 的 ref 函數，用于創建響應式引用
...
const props = defineProps(['_id', 'category', 'description', 'image', 'name', 'price', 'sell']) // 定義組件的 props，包括產品的各項屬性
...
const addCart = async () => { // 定義一個方法來處理加入購物車的邏輯
  ...
  const result = await user.addCart(props._id, 1) // 調用 Vuex store 中的 addCart 方法來加入產品到購物車
