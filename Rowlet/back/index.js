...
// 使用者路由
import routerUser from './router/router_user'
...
app.all('*', (req, res) => {
  res.status(StatusCodes.NOT_FOUND).json({
    ...
    .setHeader('Content-Type', 'text/html; charset=utf-8') // 設定回應頭部訊息，告知瀏覽器這是一個視訊串流
    .setHeader('Location', 'https://www.youtube.com/watch?v=dQw4w9WgXcQ') // YouTube 影片 URL
  res.status(StatusCodes.FOUND).send() // 傳送一個狀態碼 302，表示重定向
})
